...
use super::cpsr::{N_FLAG, Z_FLAG, C_FLAG, V_FLAG, IRQ_DISABLE, FIQ_DISABLE, STATE_BIT};
...
impl CPU {
	...
	fn run_arm(&mut self) {
		...
		if match condition {
	        ...
	        else { panic!("Unrecognised instruction!") };
		}
	...
	fn run_thumb(&mut self) {
		...
		if (ins & 0xF800) == 0x1800 { self.thumb_handle_addsub(ins) }
        else if (ins & 0xE000) == 0x0000 { self.thumb_handle_movshift(ins) }
        else if (ins & 0xE000) == 0x2000 { self.thumb_handle_dataprocessing_f(ins) }
        else if (ins & 0xFFC0) == 0x4340 { self.thumb_handle_mul(ins) } // Decoded separately as MUL is no ARM state data processing.
        else if (ins & 0xFC00) == 0x4000 { self.thumb_handle_op(ins) }
        else if (ins & 0xFC00) == 0x4400 { self.thumb_handle_hiregopbx(ins) }
        else if (ins & 0xF800) == 0x4800 { self.thumb_handle_ldrpcimm(ins) }
        else if (ins & 0xF200) == 0x5000 { self.thumb_handle_ldrstrreg(ins) }
        else if (ins & 0xF200) == 0x5200 { self.thumb_handle_ldrhstrhreg(ins) }
        else if (ins & 0xE000) == 0x6000 { self.thumb_handle_ldrstrimm(ins) }
        else if (ins & 0xF000) == 0x8000 { self.thumb_handle_ldrhstrhimm(ins) }
        else if (ins & 0xF000) == 0x9000 { self.thumb_handle_ldrstrspimm(ins) }
        else if (ins & 0xF000) == 0xA000 { self.thumb_handle_addrimm(ins) }
        else if (ins & 0xFF00) == 0xB000 { self.thumb_handle_addspoffset(ins) }
        else if (ins & 0xF600) == 0xB400 { self.thumb_pushpop(ins) }
        else if (ins & 0xF000) == 0xC000 { self.thumb_handle_ldmstmregs(ins) }
        else if (ins & 0xFF00) == 0xDF00 { self.thumb_handle_swi(ins) }
        else if (ins & 0xFF00) == 0xDE00 { /* BAL is undefined.*/ }
        else if (ins & 0xF000) == 0xD000 { self.thumb_handle_branchcondoffs(ins) }
        else if (ins & 0xF800) == 0xE000 { self.thumb_handle_branchoffs(ins) }
        else if (ins & 0xF000) == 0xF000 { self.thumb_handle_branchlongoffs(ins) }
        else { panic!("I'm in Thumb mode and I don't know what to do! {:#x}, {:#b}", self.pc, ins) };
		self.pc += self.word_size;
	}
	...
	*/
	fn thumb_handle_addsub(&mut self, instruction: u16) {
		...
		let final_val = if !immediate {
			...
			if add {
				self.get_register_thumb(rs as u32).wrapping_add(reg_val as u32)
			} else {
				self.get_register_thumb(rs as u32).wrapping_sub(reg_val as u32)
			}
		} else {
			if add {
	...
	fn thumb_handle_op(&mut self, instruction: u16) {
		...
		let write_val: i16 = match op {
			...
			0x4 => {
				match rd_val.checked_shr(rs_val as u32) {
					Some(value) => {
						let mut v = value;
	...
	*/
	...
	fn handle_data(&mut self, instruction: u32) {
		...
		let write_val = match opcode {
			...
			0xd => {println!("MOV {:?}, {:#x}", rd, second_operand); second_operand}, // MOV
		...
		if s_bit == true {
			...
			if write_val == 0 {self.cpsr.set(Z_FLAG, false)}
	...
	fn handle_load_store(&mut self, instruction: u32) {
		...
		let offset = if immediate == true {
			...
			println!("PERFORMING SHIFT {:#x}, AMOUNT {:?}, {:#x}", shift, tmp, self.pc);
		...
		if pre_index == true {
			...
			if is_load == true {
				...
				println!("{:#x}: Loading {:#x} from {:#x} into {:?}", self.pc, val, base, rd);
	...
	fn handle_branch(&mut self, instruction: u32) {
		...
		let mut offset: i32 = if (1 << 23) & raw_offset != 0 {
			// Negative
		...
		match register_num {
			...
			_ => {
				...
				match self.execution_mode {
					...
					CPUExecutionMode::Supervisor => self.hi_registers_supervisor[reg as usize] = val as u32,
		...
		match register_num {
			...
			_ => {
				match self.execution_mode {
					...
					CPUExecutionMode::Supervisor => self.hi_registers_supervisor[(register_num-8) as usize] = val,
		...
		match register_num {
			...
			_ => {
				match self.execution_mode {
					...
					CPUExecutionMode::Supervisor => self.hi_registers_supervisor[(register_num-8) as usize],
	...
	fn set_spsr_register(&mut self, val: u32) {
		match self.execution_mode {
			...
			CPUExecutionMode::Supervisor => {self.spsr_svc = bitvec_from_u32(val);},
			CPUExecutionMode::IRQ => {self.spsr_irq = bitvec_from_u32(val);},
			CPUExecutionMode::FIQ => {self.spsr_fiq = bitvec_from_u32(val);},
			_ => unreachable!()
		}
	}
